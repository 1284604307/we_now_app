...
class SingleChatProvider with ChangeNotifier {
  ...
  JMConversationInfo jSingleConver;
  ...
  /// 初始化数据
  initConversation() {
    jmSingle = JMSingle.fromJson({
      "username": jmUserInfo.username, 
    });
  ...
  _createGroupConversation() async {
    jSingleConver = await Api.jMessage.createConversation(target: jmSingle);
  ...
  getHistoryMessages() async {
    isLoading = true;
    var _historyList = await jSingleConver.getHistoryMessages(
  ...
  sendTextMessage(content) {
    jSingleConver.sendTextMessage(text: content).then((msg) {
  ...
  void sendImageMessage() async {
    ...
    if (image != null) {
      jSingleConver.sendImageMessage(path: image.path).then((msg) async {
        _parseMessage(msg, false);
        totalMsgCount += 1;
      }).catchError((error) {});
    }
