...
import 'package:flutter_app2/services/helper/theme_helper.dart';

//const Color(0xFF5394FF),

class ThemeModel with ChangeNotifier {
  static const theme_ColorIndex = 'WeNow_ThemeColorIndex';
  ...
  ThemeModel() {
    /// 用户选择的明暗模式
    _userDarkMode =
        StorageManager.sharedPreferences.getBool(theme_UserDarkMode) ?? false;
...
//          textTheme: CupertinoTextThemeData(brightness: Brightness.light)
  ...
  saveTheme2Storage(bool userDarkMode, MaterialColor themeColor) async {
    ...
    await Future.wait([
      StorageManager.sharedPreferences
          .setBool(theme_UserDarkMode, userDarkMode),
      StorageManager.sharedPreferences.setInt(theme_ColorIndex, index)
    ]);
  }
  ...
  static saveFontIndex(int index) async {
    await StorageManager.sharedPreferences.setInt(theme_FontIndex, index);
  }
}
